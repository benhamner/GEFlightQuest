...
def process_day(test_day_path, cutoff_time):
    ...
    for i, row in df_test_flights.iterrows():
        df_test_flights["actual_runway_arrival"][i] = get_scheduled_arrival(row, "runway", cutoff_time)
        df_test_flights["actual_gate_arrival"][i] = get_scheduled_arrival(row, "gate", cutoff_time)
...
def run_benchmark():
    ...
    df_out.to_csv(os.path.join(os.environ["DataPath"], "GEFlight", "sub.csv"), index=False)
